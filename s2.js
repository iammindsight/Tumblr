...
! function(e) {
    e.fn.infinitescroll = function(n, o) {
        ...
        function i(n) {
            for (var o in n) return o.indexOf && o.indexOf("Selector") > -1 && 0 === e(n[o]).length ? (t("Your " + o + " found no elements."), !1) : !0
        }

        function r(e) {
            if (e.match(v) ? e.match(v)[2] : e, e.match(/^(.*?)\b2\b(.*?$)/)) e = e.match(/^(.*?)\b2\b(.*?$)/).slice(1);
            else if (e.match(/^(.*?)2(.*?$)/)) {
                if (e.match(/^(.*?page=)2(\/.*|$)/)) return e = e.match(/^(.*?page=)2(\/.*|$)/).slice(1);
                t("Trying backup next selector parse technique. Treacherous waters here, matey."), e = e.match(/^(.*?)2(.*?$)/).slice(1)
            } else {
                if (e.match(/^(.*?page=)1(\/.*|$)/)) return e = e.match(/^(.*?page=)1(\/.*|$)/).slice(1);
                t("Sorry, we couldn't parse your Next (Previous Posts) URL. Verify your the css selector points to the correct A tag. If you still get this error: yell, scream, and kindly ask for help at infinite-scroll.com."), h.isInvalidPage = !0
            }
            return e
        }

        function a() {
            return m.localMode ? e(h.container)[0].scrollHeight && e(h.container)[0].scrollHeight : e(document).height()
        }
        ...
        function c() {
            h.loadingMsg.find("img").hide().parent().find("div").html(m.donetext).animate({
                opacity: 1
            }, 2e3).fadeOut("normal"), m.errorCallback()
        }

        function s() {
            h.isDuringAjax || h.isInvalidPage || h.isDone || l(m, h) && e(document).trigger("retrieve.infscr")
        }

        function d() {
            h.isDuringAjax = !0, h.loadingMsg.appendTo(m.contentSelector).show(), e(m.navSelector).hide(), h.currPage++, t("heading into ajax", p), g = e(e(m.contentSelector).is("table") ? "<tbody/>" : "<div/>"), f = document.createDocumentFragment(), g.load(p.join(h.currPage) + " " + m.itemSelector, null, u)
        }

        function u() {
            if (h.isDone) return c(), !1;
            var n = g.children().get();
            if (0 == n.length) return e.event.trigger("ajaxError", [{
                status: 404
            }]);
            ...
            if (e(m.contentSelector)[0].appendChild(f), h.loadingMsg.fadeOut("normal"), m.animate) {
                var t = e(window).scrollTop() + e("#infscr-loading").height() + m.extraScrollPx + "px";
                e("html,body").animate({
                    scrollTop: t
                }, 800, function() {
                    h.isDuringAjax = !1
        ...
        var g, f, m = e.extend({}, e.infinitescroll.defaults, n),
            h = e.infinitescroll;
        ...
        return p ? (p = r(p), m.localMode && (e(h.container)[0].scrollTop = 0), h.pixelsFromNavToBottom = a() + (h.container == document.documentElement ? 0 : e(h.container).offset().top) - e(m.navSelector).offset().top, h.loadingMsg = e('<div id="infscr-loading" style="text-align: center;"><img alt="Loading..." src="' + m.loadingImg + '" /><div>' + m.loadingText + "</div></div>"), (new Image).src = m.loadingImg, e(document).ajaxError(function(n, o) {
            t("Page not found. Self-destructing..."), 404 == o.status && (c(), h.isDone = !0, e(m.localMode ? this : window).unbind("scroll.infscr"))
        }), e(m.localMode ? this : window).bind("scroll.infscr", s).trigger("scroll.infscr"), e(document).bind("retrieve.infscr", d), this) : void t("Navigation selector not found")
    }, e.infinitescroll = {
        defaults: {
            debug: !1,
            preload: !1,
...
function show_tt() {
    $(window).scrollTop() > 200 ? tt.css("opacity", "1") : tt.css("opacity", "0")
}

function sidebar_left_scroll() {
    var t = $(window).scrollLeft();
    body.hasClass("l-s") && container_width > window_width ? head.css({
        position: "relative",
        left: -t + "px"
    }) : head.css({
        left: "auto"
...
function article_setup() {
    $("p").remove(":contains('Source:'),:contains('via ')"), $(".photoset-grid").photosetGrid({
        highresLinks: !0,
        rel: $(".photoset-grid").attr("data-id"),
        gutter: "5px",
        onComplete: function() {}
...
var window_width = $(window).width(),
    container_width = $("#C").innerWidth(),
    body = $("body"),
    grid = $("#Z"),
    head = $("#W"),
    head_inner = $("#W-C");
if (tt = $("#TT"), show_tt(), $(window).scroll(show_tt), tt.click(function() {
        $("html,body").animate({
            scrollTop: body.offset().top
        }, 600)
    }), sidebar_left_scroll(), $(window).scroll(sidebar_left_scroll), grid.css("min-height", $(window).height() + "px").imagesLoaded(function() {
        setTimeout(function() {
            grid.css("opacity", 1), $("#P").css("opacity", "1"), head.css("opacity", 1)
        }, 100)
    }), body.hasClass("head_inner_center") && head_inner.addClass("middle"), article_setup(), body.is("#i")) {
    var ig_user_id_attr = body.attr("data-ig-id"),
        ig_user_token = body.attr("data-ig-token");
    if (/^\d+$/.test(ig_user_id_attr)) var ig_user_id = parseInt(body.data("ig-id"));
    else var ig_user_id = !1;
    if (0 != ig_user_id) {
        if (body.hasClass("l-h")) var target = "I_F_H";
        else var target = "I_F_S";
        ...
        userFeed.run()
    } else $(".I-f-t").after("<p><u>An error processing your Instagram ID, please head over to the support page on Zen Themes for detailed instructions.</u></p>");
    grid.imagesLoaded(function() {
        setTimeout(function() {
            grid.masonry({
                itemSelector: "#post-info,.X",
                columnWidth: 1,
                isFitWidth: !0,
                transitionDuration: 0
            })
        }, 100)
    }), body.hasClass("is") && grid.infinitescroll({
        navSelector: "#P",
        nextSelector: "#P a#older",
        itemSelector: ".X",
        bufferPX: 500,
        loading: {
            finishedMsg: function() {
                $("#infscr-loading").css("opacity", "0")
            }
        }
    }, function(t) {
        var e = $(t).addClass("loading"),
            i = e.map(function() {
                return this.id
            }).get();
